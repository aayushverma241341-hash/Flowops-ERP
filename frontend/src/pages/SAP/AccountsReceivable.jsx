...
import { ArrowDownLeft, RefreshCw, Loader, FileText, Wallet } from 'lucide-react';
...
const AccountsReceivable = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const response = await api.get('/fi/ar');
    ...
    <div className="space-y-6 max-w-7xl mx-auto pb-10">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-1">Manage customer invoices and incoming payments (FB70/FBL5N).</p>
        </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden min-h-[500px]">
        {loading ? (
          <div className="flex flex-col items-center justify-center h-96 space-y-4">
            ...
            <p className="text-slate-500 font-medium">Loading receivables...</p>
          </div>
        ) : data.length === 0 ? (
              ...
              <tbody className="divide-y divide-slate-100">
                {data.map((row, idx) => (
                  <tr key={idx} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="py-4 px-6 text-sm font-medium text-slate-900">{row.customer_name}</td>
